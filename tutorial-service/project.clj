(defproject tutorial-service "0.0.1-SNAPSHOT"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "tutorial-service.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          (use 'io.pedestal.service-tools.dev)
                          (require 'tutorial-service.service)
                          ;; Nasty trick to get around being unable to reference non-clojure.core symbols in :init
                          (eval '(init tutorial-service.service/service #'tutorial-service.service/routes))
                          (catch Throwable t
